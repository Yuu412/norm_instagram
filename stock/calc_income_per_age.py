...
SUMMAEY_COLS = ['証券コード', '企業名', '平均年収', '平均年齢']
COLS = ['証券コード', '企業名', '平均年齢', '平均年収']
...
for industry_csv in indestry_csv_list:
    ...
    for index, company in df_company.iterrows():
        ...
        if((ave_income == False) or (ave_age == False)):
            continue

        if(math.isnan(ave_income) or math.isnan(ave_age)):
            continue
    ...
    df_output.to_csv(industry_path + '/income_per_age.csv', header=COLS, encoding="utf-8", index=False)